...
procedure correct_matrices(s: string; n, t, ntest, ii5: integer;
...
var
string_out: array [1..100] of string;
...
var
outmatr: array[1..100] of string;
matcols, matrows: integer; //rows of the matrix and their amount

var
bb: boolean;
i43, i42, ppb, pq, nummm, n_err2, ii6, ppa, pa, ii3, ii4, ii2, pp1, pp, ppa1,
ppa2, arrayin1, xx1, xx2: integer;
sp, si2: string;
...
var
arrayin0, matrixin0, arrayout0, matrixout0, amount_of_loops: integer;
...
begin
  ...
  if (chosen_task = '15_32-002') or (chosen_task = '15_32-004') then
  begin
    if si[1] = '.' then
      si := '-' + si;
    if en_rus then
      form2.memo1.Lines.Add('the file: ' + copy(si, 1, length(si) - 2))
    else
      form2.memo1.Lines.Add('файл: ' + copy(si, 1, length(si) - 2));
    if en_rus then
      form2.memo1.Lines.Add('the character: ' + si[length(si)])
    else
      form2.memo1.Lines.Add('символ: ' + si[length(si)]);
    exit;
  end;
  ...
  begin
    j := pos(' ', si);
    if en_rus then
      form2.memo1.Lines.add('the key: ' + copy(si, 1, j - 1))
    else
      form2.Memo1.Lines.add('ключ: ' + copy(si, 1, j - 1));
    Delete(si, 1, j);
    delete_blanks(si);
    if si = '' then
      si := '-';
  ...
  begin
    ...
    begin
      if en_rus then
        form2.memo1.Lines.add('the file for comparing: ' + copy(si, 1, j - 1))
      else
        form2.Memo1.Lines.add('файл для сравнения: ' + copy(si, 1, j - 1));
      Delete(si, 1, j);
    end;
  end;
  repeat
    delete_blanks(si);
    if (si <> '') and ((si[1] = '!') and ((filein = 1) or (k > 1)) or (copy(si, 1, 6) = '-10000') and
      (filein = 2)) then
      si := '-' + si;

    if (k = 1) and (filein = 2) then
      j := pos('-10000', si)
    else
      j := pos('!', si);
    ...
    if en_rus then
      form2.memo1.Lines.add(ae[i] + sb + sa)
    else
      form2.memo1.Lines.add(ar[i] + sb + sa);
    Inc(i);
  until si = '';
end;
...
var
  ...
  s1, s2: string;
begin
  if (copy(chosen_task, 1, 5) = '15_63') and (k = 3) and (s = 'Too many records.') then
  begin
    if not en_rus then
      s := 'Количество записей больше максимально допустимого.';
  end;
  ...
  if ((chosen_task = '15_61') or (chosen_task = '15_62')) and (s <> '') and (s[length(s)] = '#') then
    Delete(s, length(s), 1);

  s1 := s;
  if bc then
  begin
    s2 := copy(s1, q1 + 1, length(s1));
    s1 := copy(s1, 1, q1 - 1);
  end;
  //showmessage(s1+'   '+s2);
  ...
  begin
    ...
    case q2 of
      ...
      begin
        if en_rus then
          form2.Memo1.Lines.Add('The second file: ')
        else
          form2.Memo1.Lines.Add('Втоpoй файл: ');
      end;
      1:
      begin
        if bc then
          if en_rus then
            form2.Memo1.Lines.Add('The first file: ')
          else
            form2.Memo1.Lines.Add('Первый файл: ');
      end;
    end;{of case}

    if s = '' then
      if en_rus then
        form2.Memo1.Lines.add('<Empty file>')
      else
        form2.Memo1.Lines.add('<Пустoй фaйл>')
    else
      repeat
        p := pos(chr(2), s);
        if p = 1 then
        begin
          if en_rus then
            form2.Memo1.Lines.add('<Empty line>')
          else
            form2.Memo1.Lines.add('<Пустая строка>');
          Delete(s, 1, 1);
          continue;
        end;
        ...
        begin
          if not ((pos('Too', s) > 0) and (pos('many', s) > 0) and (pos('records', s) > 0) or
            (pos('записей', s) > 0) and (pos('больше', s) > 0) and
            (pos('максимально', s) > 0)) then
            addblanks(s)
          else if k = 3 then
              s := s + '.'
        ...
        begin
          form2.memo1.Lines.Add(copy(s, 1, p - 1));
          Delete(s, 1, p);
        end;
      until s = '';
  end;
...
begin
  ...
  for i := q to 13 do
    s1 := s1 + ' ';
  q := pos(' ', s);
  if (s = '') or (q = 0) or (not (s[1] in ['A'..'Z'])) then
  begin
    s := s1 + s;
    exit;
  end;
...
begin
  ...
  srr := sr;
  soo := so;
  if length(sr) <> length(so) then
    exit;
  for i := 1 to length(srr) - 4 do
  begin
    if (srr[i] = ' ') and (soo[i] = ' ') then
      continue;
    if (Ord(srr[i]) = Ord(soo[i]) - 1) and not k then
    begin
      k := True;
      srr[i] := chr(Ord(srr[i]) + 1);
      continue;
    end;
    if not k and (srr[i] <> soo[i]) then
      exit;
    if k then
      if not (srr[i] = '9') and (soo[i] = '0') then
        exit
      else
        srr[i] := '0';
  end;
...
begin
  ...
  if sr = '' then
    exit;
  if (pos(sr[1], ruscapsmall) > 0) or (chosen_task = '7_24') or
    (copy(chosen_task, 1, 5) = '15_63') then
    goto 1;
  if (length(sr) = 5) and (sr[1] in ['e', 'E']) and (sr[2] in ['r', 'R']) and
    (sr[3] in ['r', 'R']) and (sr[4] in ['o', 'O']) and (sr[5] in ['r', 'R']) then
    b := True;
  if (length(sr) = 8) and (sr[1] in ['n', 'N']) and (sr[2] in ['o', 'O']) and
    (sr[3] = ' ') and (sr[4] in ['R', 'r']) and (sr[5] in ['o', 'O']) and
    (sr[6] in ['o', 'O']) and (sr[7] in ['t', 'T']) and (sr[8] in ['s', 'S']) then
    b := True;
  if not b then
    exit;
  1:
    ...
    begin
      if (i = 1) and (sr[1] in ['a'..'z']) or (i > 1) and (sr[i] in ['A'..'Z']) then
        sr[i] := chr(Ord(sr[i]) + 32 - 64 * Ord(i = 1));
      p := pos(sr[i], ruscapsmall);
      if (i = 1) and (p in [34..66]) or (i > 1) and (p in [1..33]) then
        sr[i] := ruscapsmall[p + 33 - 66 * Ord(i = 1)];
    end;
  //showmessage(sr);
end;
...
begin
  for i := 1 to length(sr) do
    if sr[i] in ['A'..'Z'] then
      sr[i] := char(Ord(sr[i]) + 32);
  for i := 1 to length(sr) - 1 do
    if (sr[i] in ['a'..'z']) and (sr[i + 1] in ['0'..'9']) then
    begin
      sr := copy(sr, 1, i) + ' ' + copy(sr, i + 1, length(sr) - i);
      break;
    end;
end;
...
const
  maxo = 100;
var
  a, b, d: array[1..maxo] of real;
  c: array[1..maxo] of boolean;
  k5, i, j, k3, q, k4, p, k1, k2: integer;
  s: real;
  b1: boolean;
begin
  delete_blanks(si);
  ...
  for i := 1 to k1 do
  begin
    if (i < k3) and (b[i] >= b[k3]) or (i > k3) and (b[i] <= b[k3]) then
      exit;
    k4 := 0;
    for j := 1 to k1 do
      if a[j] = b[i] then
...
begin
  ...
  begin
    b := True;
    for i := 1 to length(sr) - 1 do
      if (sr[i] in ['A'..'Z', 'a'..'z']) and (sr[i + 1] in ['A'..'Z', 'a'..'z']) then
      begin
        sr := copy(sr, 1, i) + ' ' + copy(sr, i + 1, 200);
        b := False;
        break;
      end;
  end;
...
begin
  if en_rus then
    string_out[1] := 'Initial data:'
  else
    string_out[1] := 'Исходные данные:';
  if ii5 = 1 then
  begin
    if en_rus then
      string_out[2] := 'the first string: '
    else
      string_out[2] := 'первaя строка: ';
    string_out[2] := string_out[2] + copy(s, 1, length(s) div 2);
    if en_rus then
      string_out[3] := 'the second string: '
    else
      string_out[3] := 'вторая строка: ';
  ...
  begin
    ...
    if p = 0 then
    begin
      ShowMessage('Error! No ''/n'' in tests for the subroutine.');
      p := 1;
    end;
    if en_rus then
      string_out[2] := 'the string: ' + copy(s, 1, p - 1)
    else
      string_out[2] := 'строка: ' + copy(s, 1, p - 1);
    ...
    if length(s) < p + 2 then
      s := s + '!!!!!!!!';
    if en_rus then
      string_out[3] := 'substring bounds: ' + copy(s, 1, p - 1)
    else
      string_out[3] := 'грaницы пoдстpoки: ' + copy(s, 1, p - 1);
    if en_rus then
      string_out[4] := 'character range: ' + s[p + 1] + ' ' + s[p + 2]
    else
      string_out[4] := 'диапазон символов: ' + s[p + 1] + ' ' + s[p + 2];
...
begin
  ...
  if en_rus then
    string_out[1] := 'The first array: ' + s
  else
    string_out[1] := 'Первый массив: ' + s;
  s := '';
  for i := 1 to consts[it, 1] do
  begin
    p := pos(' ', si);
  ...
  if en_rus then
    string_out[2] := 'The second array: ' + s
  else
    string_out[2] := 'Втоpoй массив: ' + s;
  if en_rus then
    string_out[3] := 'The range of indices: ' + si
  else
    string_out[3] := 'Диапазон индексов : ' + si;
end;
...
begin
  ...
  if (p = 0) or (q = 0) then
    exit;
  val(copy(sr, 1, p - 1), i1, pp);
  if pp <> 0 then
    exit;
  val(copy(sr, p + 1, 100), j1, pp);
  if pp <> 0 then
    exit;
  val(copy(so, 1, q - 1), i2, pp);
  if pp <> 0 then
    exit;
  val(copy(so, q + 1, 100), j2, pp);
  if pp <> 0 then
    exit;
  if i1 * j2 = i2 * j1 then
  begin
...
begin
  i := pos('!', s);
  if (s <> '') and (i > 0) and ((chosen_task = '16_16-001') or
    (chosen_task = '16_16-004') or (chosen_task = '16_16-007')) then
  begin
    if en_rus then
      s := copy(s, 1, i - 2) + '.' + ' The pointers to the strings were changed. It is not allowed.'
    else
      s := copy(s, 1, i - 2) + '.' +
        ' Были изменены указатели в массиве ссылок, что недопустимо.';
  end;
  if (s <> '') and (i > 0) and (chosen_task <> '16_16-001') and (chosen_task <> '16_16-007') and
    (chosen_task <> '16_16-004') then
  begin
    if en_rus then
      s := copy(s, 1, i - 2) + '.' + ' The subroutine changed pointers to the strings incorrectly.'
    else
      s := copy(s, 1, i - 2) + '.' +
        ' В подпрограмме были некорректно изменены значения указателeй в массиве ссылок.';
  end;
end;
...
begin
  ...
  if i > 0 then
    if en_rus then
      s := copy(s, 1, i - 2) + '.' +
        ' The pointer to the first node of the list was changed. It is not allowed.'
    else
      s := copy(s, 1, i - 2) + '.' +
        ' Была изменена ссылка на первое звено списка, что недопустимо.';
end;
...
begin
  ...
  if check_new and not check_dispose and (p1 > 0) then
  begin
    s := copy(s, 1, p1 - 1);
    s1 := s;
    delete_blanks(s1);
    if s1 <> '' then
      s := s + '.';
    if en_rus then
      s := s + ' The New procedure was called a wrong number of times.'
    else
      s := s + ' Было зафиксировано неверное количество обращений к процедуре New.';
  end;
  if check_dispose and not check_new and (p1 > 0) then
  begin
    s := copy(s, 1, p1 - 1);
    s1 := s;
    delete_blanks(s1);
    if s1 <> '' then
      s := s + '.';
    if en_rus then
      s := s + ' The Dispose procedure was called a wrong number of times.'
    else
      s := s + ' Было зафиксировано неверное количество обращений к процедуре Dispose.';
  end;
  if check_dispose and check_new and (p1 > 0) then
  begin
    s := copy(s, 1, p1 - 1);
    s1 := s;
    delete_blanks(s1);
    if s1 <> '' then
      s := s + '.';
    if en_rus then
      s := s + ' The New and Dispose procedures were called a wrong number of times.'
    else
...
begin
  ...
  s2 := '';
  s3 := '';
  if (length(s) >= 3) and (s[length(s)] = '0') then
...
begin
  ...
  if not b1 then
    ...
    form2.memo1.Lines.add(copy(s1, 1, length(s1) - 1) + '.');
  if not b2 then
    form2.memo1.Lines.add(s4 + ' -')
  else
    form2.memo1.Lines.add(copy(s4, 1, length(s4) - 1) + '.');
end;
...
begin
  ...
  if length(sa) > 64 then
    form2.Memo1.Lines.Add(copy(sa, 65, length(sa)));
  //showmessage('@@');
end;
...
begin
  ...
  repeat
    Inc(k);
    if (sr[i] in ['2'..'5']) and (length(sr) > i) and not (sr[i + 1] = '.') and
      (i > 1) and not (sr[i - 1] = '.') then
    begin
      insert('.0', sr, i + 1);
    end;
    if (sr[i] in ['0'..'9']) and (length(sr) > i) and (sr[i + 1] = '0') then
    begin
      Delete(sr, i + 1, 1);
      Dec(i);
    end;
...
begin
  ...
  if (s <> '') and (s[length(s)] = '.') then
    Delete(s, length(s), 1);
  if (chosen_task = '15_50-002') and (s <> '') then
...
begin
  ...
  case t of
    0: if en_rus then
        ...
        string_out[1] := 'Исходные данные: ';
    1: if en_rus then
        string_out[1] := 'Obtained result: '
      else
        string_out[1] := 'Полученный результат: ';
    2: if en_rus then
        string_out[1] := 'Correct result: '
      else
        string_out[1] := 'Правильный результат: ';
  end;{case}
  ...
  begin
    ...
    Inc(k);
    if (k = m div n) or (s = '') then
    begin
      k := 0;
      Inc(q);
      case q of
        1: if en_rus then
            s1 := 'the first array: ' + s1
          else
            s1 := 'первый массив: ' + s1;
        2: if en_rus then
            s1 := 'the second array: ' + s1
          else
            s1 := 'второй массив: ' + s1;
        3: if en_rus then
            s1 := 'the third array: ' + s1
          else
            s1 := 'третий массив: ' + s1;
        4: if en_rus then
            s1 := 'the fourth array: ' + s1
          else
            s1 := 'четвертый массив: ' + s1;
      end;
      if q = n then
      begin
        string_out[1 + q] := s1 + s;
...
// output number_of_lines - how many lines were formed
var
  j, j0, matcols, matrows, p, j3, j1, j2, ii2: integer;
  rr2: real;
  si2, si1, s1: string;
  bi: integer;
begin
  ...
  si1 := s;
  bi := -1;
  if result_type[ii5, 1] = 'r' then
    bi := 0;  //it is one but not L
  if result_type[ii5, 1] = 'i' then
    bi := 1;
  if bi = -1 then
    bi := Ord(pos('.', s) = 0);
    ...
    begin
      case j0 of
        ...
        3: if en_rus then
            ...
            s1 := 'третья матрица: ';
        4: if en_rus then
            s1 := 'the fourth matrix: '
          else
            s1 := 'четвертая матрица: ';
      end;
      Inc(j);
      string_out[j] := s1;
    end;

    for j1 := 1 to matrows do
    begin
      Inc(j);
      string_out[j] := '';
      ...
      begin
        ...
        begin
          si2 := copy(si1, 1, j3 - 1);
          if bi = 1 then
            val(copy(si1, 1, j3 - 1), ii2, p)
          else
            val(copy(si1, 1, j3 - 1), rr2, p);
          si1 := copy(si1, j3 + 1, 10000);
        end
        else
        begin
          si2 := si1;
        ...
        begin
          if p = 0 then
            ...
            str(rr2: 16 + 3 * byte(j2 > matcols): 5, si2);
...
const
  eps = 0.000001;
var
  r3, r1, r2, accuracy: real;
  ia1, ic1, pb, pp, l, p1, p2, j, i, i1, i2: integer;
  sf, s1, s2: string;
  iii: integer;
  bbb: boolean;
begin
  ...
  if (chosen_task <> '6_24') and (chosen_task <> '15_25') and (textsout = 0) then
  begin
    delete_blanks(so);
    delete_blanks(sr);
  end;
  if (chosen_task = '15_54') or (chosen_task = '15_58') or (copy(chosen_task, 1, 5) = '15_63') then
  begin
    delete_blanks(so);
    delete_blanks(sr);
  end;
  //showmessage(sr);
  if chosen_task = '8_29-008' then
  begin
    diff := analysis_8_29_8(si, sr, so);
    exit;
  end;
  ...
  if (chosen_task = '7_24') or (errorr)
  // and ((copy(sr,1,5)='error')or(copy(sr,1,8)='no roots')))
  then
  begin
    correctcapitalsmall(sr);//showmessage(sr);
  end;

  if (run_time_err) and (pos('untime error ', sr) > 0) and (pos(' at ', sr) > 10) then
  begin
    sf := sr;
    pp := 0;
    repeat
      Inc(pp);
    until (length(sf) < pp) or (sf[pp] in ['1'..'9']);

    //showmessage(sf+'    !  '+inttostr(pp));

    if sf[pp] in ['1'..'9'] then
    begin
      sf := copy(sf, pp, 10);
      i11 := 1;
      while (i11 < length(sf)) and (sf[i11] <> ' ') do
        Inc(i11);
      val(copy(sf, 1, i11 - 1), pb, pp);

      //showmessage(sf+'  '+inttostr(pb)+'  '+inttostr(i11));

      if ((so = 'Runtime error ' + IntToStr(pb)) or (so = 'runtime error ' + IntToStr(pb))) and
        (pos('Runtime error ' + IntToStr(pb), sr) > 0) then
      begin
        diff := 0;
        exit;
      end;
  ...
  if errorr then
    if (so = 'Error') and ((sr = 'Error') or (sr = 'Ошибка')) then
    begin
      diff := 0;
      exit;
    end;

  if chosen_task = '4_12-005' then
  begin
    if ((sr = 'No roots') or (copy(sr, 1, 3) = 'Нет')) and (so = 'No roots') then
    begin
      diff := 0;
      //if en_rus then sr:='No roots' else begin sr:='Нет корней'; so:=sr end;
      exit;
    end;
    if ((sr = 'Error') or (sr = 'Ошибка')) and (so = 'Error') then
    begin
      diff := 0;
  ...
  if (chosen_task = '10_21') or (chosen_task = '10_22') then
    correct10_21_22(sr);

  //A special correction for the results of the tasks 14_15-002
  if (chosen_task = '14_15-002') or (chosen_task = '14_27-002') or (chosen_task = '14_27-003') then
    correct14_15_27(sr);

  if chosen_task = '15_58' then
  begin
    ic1 := correct_15_58(so);
    ia1 := correct_15_58(sr);
  ...
  if (sr <> '') and (so = '') and (pos('!', sr) > 0) and (check_new or check_dispose) then
  begin
    diff := 1;
    exit;
  end;
  if (sr <> '') and (so = '') and (pos('!', sr) > 0) and (copy(chosen_task, 1, 5) = '16_16') then
  begin
    diff := 1;
    exit;
  end;
  ...
  begin
    ...
    if (chosen_task <> '6_24') and (chosen_task <> '15_25') and
      ((textsout = 0) or (chosen_task = '15_54') or (chosen_task = '15_58')) then
    begin
      i := pos(' ', so);
      j := pos(' ', sr);
    end
    ...
    if chosen_task = '15_63-005' then
      correct15_63_005(sr);

    if (copy(chosen_task, 1, 5) = '15_63') and (so = 'Too many records.') then
    begin
      //showmessage(sr);
      ...
      if (pos('Too', sr) > 0) and (pos('many', sr) > 0) and (pos('records', sr) > 0) and
        (length(sr) < 60) or (pos('записей', sr) > 0) and (pos('больше', sr) > 0) and
        (pos('максимально', sr) > 0) and (length(sr) < 60) then
      begin
        diff := 0;
        exit;
      end;
    ...
    if (i = 0) and (j <> 0) and ((copy(chosen_task, 1, 5) = '16_18') or
      (copy(chosen_task, 1, 5) = '16_19')) and (pos('!', sr) > 0) then
    begin
      diff := 1;
      exit;
    end;

    if (i = 0) and (j <> 0) and (pos(' !', sr) > 0) and (check_new or check_dispose) then
    begin
      diff := 1;
      exit;
    end;

    if (i = 0) and (j <> 0) and (pos(' !', sr) > 0) and (copy(chosen_task, 1, 5) = '16_16') then
    begin
      diff := 1;
      exit;
    end;
    ...
    begin
      s1 := copy(so, 1, i - 1);
      so := copy(so, i + 1, 10000);
      s2 := copy(sr, 1, j - 1);
      sr := copy(sr, j + 1, 10000);
    end;
    ...
    case result_type[ii5, L] of
      ...
      'c':
      begin
        if (length(s1) <> 1) or (length(s2) <> 1) or (s1 <> s2) then
        begin
          diff := 1;
          exit;
        end;
      ...
      begin
        ...
        if (s1 = 'true') or (s1 = 'True') then
          iii := integer(pos(s2[1], 'YyДдTt') = 0)
        else if (s1 = 'false') or (s1 = 'False') then
            iii := integer(pos(s2[1], 'НнFfNn') = 0)
          else
          begin
            diff := 1;
            exit;
      ...
      begin
        ...
        if (r1 * r2 < 0) or (r1 = 0) and (r2 <> 0) or (r2 = 0) and (r1 <> 0) then
        begin
        ...
        begin
          ...
          r2 := abs(r2);
          r3 := sqrt(r1) + sqrt(r2);
          if (abs(r3) < 1e-15) then
            accuracy := abs(r1 - r2)
          else
            accuracy := 2 * abs(r1 - r2) / r3;
          //showmessage (floattostr(accuracy));
          //showmessage (floattostr(r1));
          //showmessage (floattostr(r2));
          //showmessage (floattostr(r3));
          if (accuracy < 100 * eps) and (accuracy > eps) then
          begin
            diff := 2;
            exit;
          end
          else
            if (accuracy >= 100 * eps) then
            begin
              diff := 1;
              exit;
            end;
        ...
        ShowMessage('Wrong type of result for the test ' + IntToStr(n_test) + '.');
        halt
...
begin
  ...
  if (chosen_task = '16_16-003') or (chosen_task = '16_16-005') or
    (chosen_task = '16_16-004') or (chosen_task = '16_16-006') or (chosen_task = '16_16-007') then
    if en_rus then
      form2.Memo1.Lines.Add('The text:')
    else
      form2.Memo1.Lines.Add('Teкcт:');

  if (chosen_task = '16_16-003') or (chosen_task = '16_16-004') or
    (chosen_task = '16_16-005') or (chosen_task = '16_16-006') or (chosen_task = '16_16-007') then
  begin
    if (s <> '') and (s[1] = '.') then
      if en_rus then
        form2.memo1.Lines.add('<Empty text>')
      else
        form2.memo1.Lines.add('<Пустой текст>');
    p := pos('.', s);
    Delete(s, p, 1);
    p := pos(' ', s);
    if p = 0 then
...
begin
  ...
  begin
    if en_rus then
      ...
      form2.Memo1.Lines.Add('<Пустoй тeкст>');
  end
  else
    for j := 1 to length(s) div consts[(i - 1) mod 4 + 1, 2] do
      form2.Memo1.Lines.Add(copy(s, 1 + (j - 1) * consts[(i - 1) mod 4 + 1, 2], consts[(i - 1) mod 4 + 1, 2]));
  if s2 <> '' then
    form2.Memo1.Lines.Add(s2);
end;
...
//This procedure uses a global variable I that is the number of the current test.
var
  p, j3, j1, j2, ii2: integer;
  rr2: real;
  si1, si2, si3: string;
  bi: integer;
begin
  while (sf <> '') and (sf[1] = ' ') do
    sf := copy(sf, 2, 10000);
  while (sf <> '') and (sf[length(sf)] = ' ') do
    sf := copy(sf, 1, length(sf) - 1);
  ...
  bi := byte((copy(chosen_task, 1, 3) = '9_5') or (copy(chosen_task, 1, 4) = '9_18') or
    (copy(chosen_task, 1, 4) = '9_19') or (chosen_task = '9_25') or (chosen_task = '9_27') or
    (chosen_task = '9_31')) + 2 * byte((chosen_task = '9_20') or (chosen_task = '9_21'));
  ...
  begin
    ...
    begin
      si3 := '';
      for j2 := 1 to matcols + byte(chosen_task = '9_33') do
      begin
        while (si1 <> '') and (si1[1] = ' ') do
          si1 := copy(si1, 2, 10000);
        if (chosen_task = '9_33') and (j2 < j1) then
          si2 := '              '
        else
        begin
          j3 := pos(' ', si1);
          ...
          begin
            if p = 0 then
              ...
              str(rr2: 15 + 3 * byte(j2 > matcols): 5, si2);
        end;
        si3 := si3 + si2;
        if si1 = '' then
          break;
  ...
  begin
    ...
    begin
      ...
      if j3 = 0 then
        ...
        sf := copy(sf, 1, j3 - 1) + copy(sf, j3 + 1, 10000);
    end;{of while}
    j3 := 1;
    while j3 < length(sf) do
    begin
      sf := copy(sf, 1, j3) + ' ' + copy(sf, j3 + 1, 10000);
      j3 := j3 + 2;
    end;
    ...
    for j1 := 1 to matrows do
      if j1 < matrows then
      begin
        outmatr[j1] := copy(sf, 1, 2 * matcols);
        sf := copy(sf, 2 * matcols + 1, 10000);
      end
      else
        outmatr[j1] := sf;
  end;
...
var
  srr: string;
  k, i, n, p, j: integer;
  r: array[1..max_test_number] of real;
  s: real;
begin
  n := 0;
  delete_blanks(sr);
...
begin
  ...
  form2.Memo2.Clear;
  form2.memo2.Width := _width * 2;//i2:=0;
  for i := 1 to 2 * max_test_number do
  begin
    k1 := linetest[i] - 1;
    k2 := linetest[i + 1] - 2;
    b := k2 <= 0;
    if k2 <= 0 then
      k2 := form2.Memo1.Lines.Count - 2;
    while (k2 > 0) and ((copy(form2.memo1.Lines[k2], 1, 2) = '  ') or (form2.memo1.Lines[k2] = ' ') or
        (copy(form2.memo1.Lines[k2], 1, 7) = 'Warning') or
        (copy(form2.memo1.Lines[k2], 1, 7) = 'Предупр')) or
      (copy(form2.memo1.Lines[k2], 1, 5) = 'Hint:') or
      (copy(form2.memo1.Lines[k2], 1, 7) = 'Замечан') do
      Dec(k2);

    if (form2.Memo1.Lines[k2] <> 'Ok') then
      for j1 := k1 to k2 do
      begin
        s := form2.Memo1.Lines[j1];//delete_blanks(s);
        //p:=pos(' ',s); if p>0 then delete(s,p,1);
        form2.Memo2.Lines.add(s);
      end;
    if linetest[i + 1] <= 0 then
      break;
  end;
  j := -1;
  for i := form2.Memo1.Lines.Count - 1 downto form2.Memo1.Lines.Count div 2 do
    if (copy(form2.Memo1.Lines[i], 1, 7) = 'Warning') or
      (copy(form2.Memo1.Lines[i], 1, 7) = 'Предупр') or
      (copy(form2.memo1.Lines[i], 1, 5) = 'Hint:') or
      (copy(form2.memo1.Lines[i], 1, 7) = 'Замечан') then
      j := i;
  if j > -1 then
    for i := j to form2.Memo1.Lines.Count - 1 do
      form2.Memo2.Lines.Add(form2.Memo1.Lines[i]);
  form2.Memo1.Clear;
  for i := 1 to form2.Memo2.Lines.Count do
  begin
